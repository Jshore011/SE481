...
public class Game extends JFrame implements IRun{
	...
	public void startGame() {
		...
		//this.time = (IClock) new Clock(FRAMES_PER_SECOND);
		IConfigurationElement[] config = Platform.getExtensionRegistry()
		.getConfigurationElementsFor("asteroidplugin2.IClockextensionpoint");//Change to your own
		String pluginId = config[0].getContributor().getName();

		System.out.println("The plug in ID: " + pluginId);
		try {
			for (IConfigurationElement e : config) {
				System.out.println("Evaluating Clock extension");
				final Object o = e.createExecutableExtension("iclock");
				if (o instanceof IClock) {
					time = ((IClock)o);
					time.setCyclesPerSecond(FRAMES_PER_SECOND);
					time.reset();
					//((IClock) o).setCyclesPerSecond(FRAMES_PER_SECOND);
		...
		while(true) {
			...
			time.update();
			for(int i = 0; i < 5 && time.hasElapsedCycle(); i++) {
				updateGame();
			}
			
			//Repaint the window.
			((Component) world).repaint();
